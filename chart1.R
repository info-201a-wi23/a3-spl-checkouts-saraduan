...
checkouts$date <- as.Date(checkouts$date, format = "%Y-%m-%d")
...
video_discs <- checkouts %>% 
  filter(MaterialType %in% c("VIDEODISC")) %>%
  group_by(date) %>%
  summarize(total_checkouts = sum(Checkouts))  
video <- "Video Discs"
video_discs$MaterialType <- video
...
all_materials <- rbind(all_materials, video_discs)
...
ggplot(data = all_materials) +
  ...
  scale_color_manual(values = c("#91e5f6", "#84d2f6", "#59a5d8", "#386fa4", "#133c55")) +
  guides(lwd = FALSE) +
